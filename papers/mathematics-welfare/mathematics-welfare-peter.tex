...
\usepackage[colorlinks=true,allcolors=blue,pdftitle={Misaligned or Undersupplied? Pure Mathematics in the Age of LLMs},pdfauthor={Peter Cotton}]{hyperref}
...
\title{\textbf{Misaligned or Undersupplied?\\ Pure Mathematics in the Age of LLMs}}
\author{Peter Cotton}
\date{September 2026}
...
\section{Misaligned, or undersupplied}

Twenty-five Fields medalists have signed a declaration, \emph{A Severe Misalignment of AI in Mathematics} \citep{avila2026}. Its complaint is that the push to solve mathematical problems as a benchmark is detrimental to the science: solving is a proxy for understanding, the mass production of true/false statements could destroy fertile ground instead of breathing life into new ideas, and the human chain of talks, discussions, and simplifications that turns a difficult proof into a textbook chapter could be lost. The declaration allows, in one sentence, that AI offers the potential of enhancing and accelerating genuine mathematical study and understanding.

This note is directed at an old related question: how much should society invest in pure mathematical research? I will argue mainly for a {\em relative} adjustment to whatever one's pre-standing opinion might be, due to the arrival of large language models. However I confess to a strong prejudice regarding the absolute level as well.

The ratio in question is that of investment in {\em unmotivated} mathematics to investment in other professions. By unmotivated mathematics I mean research whose direction is chosen by criteria internal to mathematics: generality, difficulty, elegance, the desire to find connections, and a taste for efficiency in assumptions that some in other fields might regard as premature abstraction. Pure mathematics is the obvious case. So is a large share of applied mathematics. Radon had no client in 1917 for the transform that now runs every CT scanner. Sinkhorn was proving a theorem about doubly stochastic matrices, not balancing betting pools.

The defining property is that the eventual user is not known at the time of the work. \citet{wigner1960} called the fit unreasonable; here it is an expectation over possible users, and the expectation is governed by how efficiently results travel.

The label ``unmotivated'' is a deliberate misnomer. Work chosen for generality, elegance, and difficulty is not work chosen without a motive. It is work chosen by a rule that maximizes expected value over uses nobody can yet name. The internal criteria of mathematics are in that sense a societal optimization, one that individual mathematicians carry out without needing to know it.

The argument runs as follows.
\begin{enumerate}
\item Transmission was the bottleneck, not the mathematics. All eight fields in the survey with a named dismissal later found use, the median wait from a field's founding to first external use was 68 years across 33 dated fields, and nine documented reinventions show why: a result reached a use only when someone along the way knew both the problem and the theorem, a bilingual person.
\item A language model is that person, at every use, the same day. What it performs is abstraction, not retrieval, and because it names the same thing the same it is one bilingual person for every pair of fields at once. It is tested three ways.
\item So the marginal result is worth a multiple, and the gap between what society should spend and what private returns will pay for has widened, since the new uses are unpriced. The multiple is illustrated, not measured, and the direction follows under assumptions the note states: cheap screening, transport that carries to the marginal result, and social value that rises faster than what producers capture.
\item Meanwhile the supply is being cut, and the medalists' letter aims at the wrong target. Their misalignment is real and is inside mathematics, in incentives and attribution. The larger fact is outside it.
\item Society is underinvesting in pure mathematics, and by more than it was.
\end{enumerate}

\section{A personal aside}

The pure mathematics class I graduated with at the University of New South Wales numbered about half a dozen. Down the hill, law and commerce enrolled students by the thousand; today the law faculty has about 2{,}600 students \citep{unswlaw} and the business school about 24{,}000 \citep{unswbiz}. Nobody found this odd. The ratio looked like a market outcome: the professions paid, and pure mathematics, on the whole, did not. I tell the story to make one point: whatever the right level of investment in the production of pure mathematics is, nobody looking at six students could believe Australia was above it.\footnote{Especially if they knew that none would go on to become pure mathematicians.}
...
At the time, I recall being persuaded against further pursuit of mathematics. The professors who taught me bemoaned a gutting of the profession, something that felt like a halving of real income over a generation. I did pursue a doctorate, as it happens, but neither I nor anyone else from that small class went on to work on unmotivated mathematics. Australia is not necessarily representative, but I suspect not entirely unusual either.

The loss was of standing more than of purchasing power, and the government of the day was not a friend.\footnote{Adjusted for inflation, an Australian professor's salary fell 12 per cent between 1977 and 2002, most of it before 1990; against average weekly earnings it fell from 3.2 times the average to 2.4 \citep{robinson2005}, on the series compiled by \citet{horsley2002}. A vice-chancellor at an elite Australian university earned about 2.9 times a lecturer's pay in 1975 and 16 times in 2018 \citep{rowlands2020}. The Dawkins reforms of 1987 to 1990 amalgamated institutions by legislative force, remade governing bodies on the model of company boards, and steered funding toward fields of importance to economic recovery \citep{dawkins1988}; the 1996 budget cut operating grants by up to 12 per cent \citep{the1996}; and a study of seventeen universities a decade later found an enterprise university that too often worked around and against academic cultures \citep{marginson2000}. Real faculty salaries in the United States in 1990--91 were about 8 per cent below 1971--72 \citep{aha1991}.}

That was the profession I was being warned away from. I spent the next three decades as an applied mathematician instead, albeit one with a love and respect for pure mathematics, and in that time I have witnessed on countless occasions the failure of businesses to recognize the abstract structure of their operations or the benefit that might accrue from appropriate framing. Examples include the market-maker who does not realize they are solving a control problem and denies themselves a rich set of techniques, or the data teams who perform unwitting filtering or Bayesian statistics.

My profession is littered with ad hoc attempts at everything from shrinkage to harmonic analysis, and yet it is regarded as one of the more quantitative fields, by far. The default is to {\em never} formalize. To suggest that mathematics has had a last mile problem is, in my view, an understatement.

Mathematics fails even to travel to adjacent theoretical fields. Backpropagation, Gaussian process regression, Kalman filtering, attention, and mirror descent all had near-identical predecessors under other names.

Our old topic has unusual new urgency, and given the societal debt, the irritation to pure mathematicians caused by nonchalant announcements of AI-generated counter-examples is more than understandable. Finding and inventing new structure is difficult. Working at the highest level of abstraction is physically hard. (Too hard for me, as it happens. I moved to applied work early on in my Ph.D.)

\section{Transmission was the bottleneck, until now}
...
The historical record on travel time to application is less encouraging than the probability of eventual use, admittedly. A survey of the pure-mathematics fields in the 1959 Mathematics Subject Classification, 33 of them with a datable founding and a survivor-selected sample by construction, finds a median gap of 68 years between a field's founding and its first documented external application, with quartiles at 37 and 107 years \citep{cotton2026pure}. That is evidence about fields, not about the probability that an individual result is ever used. Boole's algebra of 1854 waited 83 years for Shannon to notice that it described relay circuits. Part of each lag is waiting for a problem, or a complementary technology, to exist at all. The rest is transmission, which until recently meant a person who happened to know both the problem and the theorem, a bilingual person.

\citet{hardy1940} wrote that number theory would never be useful. It became public-key cryptography in 1977, within his readers' lifetimes and 176 years after Gauss's \emph{Disquisitiones}. He was wrong about the destination, but not about the mechanism as it stood in 1940: the results reached only other number theorists, and the odds that one of them would be in the room when the cryptographers needed a trapdoor were poor. Those odds are now close to one.

The transmission claim rests on documented cases in which the same result was invented more than once, under different names, in fields that did not read each other. Nine follow, compressed from \citet{cotton2026protocol}.

\paragraph{One invariant, four names.} Given a matrix $X$, matroid theory defined the girth of its vector matroid, compressed sensing defined its spark, tensor decomposition defined its Kruskal rank, and coding theory defined its $k$-stability. For a given matrix, and so over one coefficient field, the identities are exact,
\begin{equation}
\operatorname{girth}(X) = \operatorname{spark}(X) = \operatorname{krank}(X) + 1,
\end{equation}
so that a hardness theorem for one is a hardness theorem for all, and uniqueness conditions for sparse recovery can be read against uniqueness conditions for tensor decomposition. Each of the three later literatures repeated theorems the first already held.

\paragraph{Factor graphs.} Coding theory had the BCJR algorithm and turbo decoding, statistics had the forward-backward algorithm and the Kalman filter, and artificial intelligence had belief propagation. All are instances of one message-passing scheme, factor a global function into local ones and pass messages that eliminate variables, and the BCJR algorithm was reinvented several times before anyone said so. The factor-graph formulation then made receiver architectures derivable that no one had composed.

\paragraph{Reverse-mode differentiation.} The principle under backpropagation, that the gradient of a scalar program costs a small multiple of the program, was published in numerical analysis, control, and statistics before neural networks, and then once more inside them. Each community derived it in its own notation and cited none of the others. The idea travelled to the field that made it famous last.

\paragraph{Expectation maximization.} Before 1977, procedures now recognized as EM had been proposed for missing observations, censored data, finite mixtures, variance components, and latent variables, each as a special trick. Dempster, Laird, and Rubin stated the common incomplete-data structure and proved the monotonicity that every special case had shared without noticing. A discussant observed that convergence arguments that were hard in the particular models became simple once generalized.

\paragraph{Mendelians and biometricians.} For two decades geneticists working with discrete factors and statisticians working with continuous traits treated their accounts of heredity as rivals. Fisher's 1918 paper showed that many small Mendelian factors plus environment give a continuous trait, so the two schools had been measuring different scales of one system. Historians warn that the dispute was also about institutions and personalities, but the mathematics that dissolved it had been available throughout.

\paragraph{Iterative proportional fitting.} The iteration that alternately rescales rows and columns of a matrix to prescribed marginals is raking in survey statistics, RAS in economics, iterative proportional fitting in contingency tables, Sinkhorn scaling in numerical analysis, and alternating information projection in transport theory. Each literature has its own convergence proofs. A choice-modelling algorithm was recently recognized as Sinkhorn's, at which point matrix-balancing theory settled questions the choice vocabulary had left open.

\paragraph{Wavelets.} Applied mathematicians built wavelet bases, signal processors built perfect-reconstruction filter banks, and computer-vision researchers built image pyramids, with substantial overlap none of them saw at first. The coefficients in the wavelet constructions were already in the filter-bank literature as quadrature-mirror sequences. Mallat's multiresolution framework connected the three, after which regularity theory, reconstruction conditions, and fast algorithms reinforced one another.

\paragraph{Kriging and Gaussian processes.} Geostatistics speaks of kriging and variograms, machine learning of Gaussian-process regression and kernels, statistics of smoothing splines, learning theory of reproducing-kernel Hilbert spaces. In standard settings the predictors coincide, and comparative studies report that the vocabularies still inhibit transfer. This one is unresolved, and it is presumably being translated by a model somewhere as this is read.

\paragraph{Spectral factorization.} The matrix Fej\'er-Riesz theorem, Wiener-Hopf factorization, spectral factorization, and polynomial sum-of-squares factorization are closely related results that operator theory, control, signal processing, and algebra developed apart for decades. Symbolic and numerical mathematicians held complementary parts of the solution. Connecting the literatures produced bit-complexity results that neither side could have reached alone.

The reinventions are one object under two names. A second failure says more about abstraction itself. A theorem states that every $A$ has property $B$. An instance $A'$ of $A$ turns up in another field, so $A'$ has property $B'$ at no cost, but nobody recognizes $A'$ as an instance of $A$, and $B'$ is proved again by hand or left open.

Figure~\ref{fig:descent} draws four cases. The collapsed-strata variance estimator and its bias theorem date from 1953; a 2025 paper on finely stratified experiments derived both again. Additive segment-cost objectives have had an exact quadratic-time solution since 1989; a 2024 paper solved one by mixed-integer programming and called exact optimality its contribution. The relative-entropy variational identity is 1970s large deviations; a 2026 control paper proved its control-affine instance as a new equivalence.

Against these, Grossmann recognized Einstein's problem as an instance of Riemannian geometry in 1912, and the whole theory transferred at once. Recognizing an instance is abstraction, and the drop in the figure costs nothing once it is done.

\begin{figure}[tbp]
\centering
\begin{tikzpicture}[x=1cm, y=1cm, >=Stealth, font=\footnotesize]
  \draw[->] (0,0) -- (2.9,0);
  \draw (3.05,-0.12) -- (3.25,0.12); \draw (3.15,-0.12) -- (3.35,0.12);
  \draw[->] (3.4,0) -- (12.6,0) node[right] {year};
  \draw[->] (0,0) -- (0,3.6) node[above] {abstraction};
  \draw (0.35,0) -- (0.35,-0.08) node[below] {1850};
  \draw (2.10,0) -- (2.10,-0.08) node[below] {1900};
  \draw (3.60,0) -- (3.60,-0.08) node[below] {1950};
  \draw (6.30,0) -- (6.30,-0.08) node[below] {1975};
  \draw (9.00,0) -- (9.00,-0.08) node[below] {2000};
  \draw (11.70,0) -- (11.70,-0.08) node[below] {2025};
  \node[left] at (0,3.00) {theorem};
  \node[left] at (0,1.00) {instance};
  \fill (0.49,3.00) circle (1.6pt); \node[above] at (0.49,3.00) {Riemann 1854};
  \draw[->,thick] (0.49,3.00) -- (2.52,1.15);
  \fill (2.52,1.00) circle (1.6pt); \node[below] at (2.52,1.00) {Grossmann 1912};
  \fill (3.92,3.00) circle (1.6pt); \node[above,yshift=0pt] at (3.92,3.00) {collapsed strata 1953};
  \draw[dashed,->] (3.92,3.00) -- (3.92,1.27);
  \draw[very thick] (3.92,1.12) -- (11.70,1.12); \fill (11.70,1.12) circle (1.6pt);
  \fill (6.30,3.00) circle (1.6pt); \node[above,yshift=9pt] at (6.30,3.00) {entropy duality 1975};
  \draw[dashed,->] (6.30,3.00) -- (6.30,1.19);
  \draw[very thick] (6.30,1.04) -- (11.81,1.04); \fill (11.81,1.04) circle (1.6pt);
  \fill (7.81,3.00) circle (1.6pt); \node[above,yshift=0pt] at (7.81,3.00) {optimal partitioning 1989};
  \draw[dashed,->] (7.81,3.00) -- (7.81,1.11);
  \draw[very thick] (7.81,0.96) -- (11.59,0.96); \fill (11.59,0.96) circle (1.6pt);
  \node[below] at (11.70,0.88) {re-derived 2024--26};
  \draw[dashed,->] (9.9,2.7) -- (10.7,2.7) node[right] {descent available, not taken};
  \draw[->,thick] (9.9,2.35) -- (10.7,2.35) node[right] {descent taken};
  \draw[very thick] (9.9,2.0) -- (10.7,2.0) node[right] {wait until proved by hand};
\end{tikzpicture}
\caption{Abstraction against time, three descents not taken and one taken. A theorem at the top; an instance of it, later and lower, in another field. The dashed drop from theorem to instance costs nothing once the instance is recognized; the thick line is how long the field waited before proving the instance by hand. Grossmann's line is the drop taken at once.}
\label{fig:descent}
\end{figure}

In every case the result had been published, indexed, and searchable for years. The missing piece was the recognition that two objects described in different vocabularies were one object, and until recently only a person could supply it. That recognition is abstraction, not retrieval, and it is now available on demand: a model that has read four literatures can be asked whether spark and girth are the same thing and answer with the identity. Journals and search engines organized text and left the abstraction to the reader. That is the part of the change most likely to be undercounted.

I tested this. I put each of the nine cases to a separate, fresh instance of Claude (Fable 5.1), asking the question a practitioner in one field would ask, in that field's terms only. The prompt for the first case was:
\begin{quote}
Answer from your own knowledge only. Do not use web search or any tools. Answer in at most 150 words. I work in compressed sensing. For a sensing matrix $X$ we use the quantity called the spark, the smallest number of columns of $X$ that are linearly dependent. Is this quantity known under any other names in other fields of mathematics or engineering, and if so, what exactly is the relationship (state any identity precisely)?
\end{quote}

All nine answers named the other literatures and stated the relationship, usually with the identity and the dates; for this prompt the answer gave matroid girth, the minimum distance of a linear code, and Kruskal rank, with $\operatorname{spark}(X) = \operatorname{krank}(X) + 1$. The prompts and answers are recorded with the paper. All nine identifications were publicly available before the model's training cutoff, the cases were chosen because the equivalence is known, and the prompt asks for other names, so this is a demonstration of a capability, not an estimate of how often a model will make a match unprompted.

\section{Literature that opines on the return on investment}

\citet{jones2020} compute the average economy-wide social return to innovation investment and find a benefit-cost ratio above ten under central parameters, at least four under conservative ones, and above twenty once international spillovers and health benefits are counted. The estimate is for research and development as a whole, not mathematics, but mathematics is at the upstream end of the chain, where the spillovers are largest.

\citet{deloitte2013} attribute 2.8 million UK jobs and \pounds 208 billion of gross value added, about sixteen per cent of the total, to occupations that use tools derived from mathematical-science research. The Academy for the Mathematical Sciences updated the figure to \pounds 495 billion for 2023 \citep{acadmathsci2024}. These are gross measures with an occupational attribution rule, not marginal returns to another pound of research, and they bound the total from above rather than estimate the return to one more pound.

\citet{nelson1959} and \citet{arrow1962} argued that private markets underinvest in basic research because the returns cannot be appropriated by the investor. \citet{romer1990} made the mechanism explicit: ideas are non-rival, so their social value scales with the number of users while the private value does not. The argument is older than large language models and does not depend on them. It implies that spending set by private returns, absent a policy correction, falls below the social optimum.

\citet{akcigit2021} estimate a growth model in which basic and applied research generate different spillovers and find that a uniform research subsidy oversubsidizes applied research, while support for public basic research is welfare-improving. \citet{mansfield1991} surveyed firms and found that about a tenth of their new products and processes could not have been developed without recent academic research, and put the social rate of return to that research near thirty per cent. \citet{salter2001} review the wider evidence and reach the same conclusion.
...
A large language model is, for present purposes, a device that makes every result in its training corpus available at every use simultaneously. The net extends to anything an LLM or agentic system can reach within minutes.

\paragraph{Reach.}  The declaration is about the production of mathematical truth. The more radical change is in idea {\em transport}.
A result is present in every copy of a sufficiently advanced model, one that also possesses the ability to manifest concrete instantiations. The recipient no longer needs to know the theorem. They need to describe the problem, and the model supplies the match. The match is that recognition, abstraction rather than retrieval.

The model solves the needle-in-a-haystack search of \citet{agrawal2019}. The encounter probability rises from the chance that a bilingual person was present to the chance that a model is consulted and makes the match. The second is below one, since retrieval and recognition both fail sometimes, but it is well above one in ten and rising. If bilingual people were present at one use in ten and a model is now consulted at nearly every use, the encounter probability rises by nearly a factor of ten; if at one in thirty, thirty.

Reach counts understate the saving. When $k$ fields hold the same result under $k$ names, transfer between any two of them needs its own translation, $k(k-1)/2$ in all, and the reinventions above are the record of translations never built. A common name reduces the count to $k$: each field translates once, to the hub, and the hub translates onward. That is what abstraction does for mathematics, and \citet{cotton2026protocol} makes the arithmetic explicit.

A model that names the same thing the same is that hub. It is one bilingual person for every pair of fields at once, rather than one per pair, and the reach multiplier does not see the difference, since it counts uses reached per result and not translations built per field. It therefore does not credit the saving in search cost.

The objection that the ideas were already free is contradicted by the record. Journals were open, or nearly so, for decades before language models, and the lags were still measured in generations. Free to read is not the same as easy to find, or easy to recognize as relevant; \citet{jaffe1993} found spillovers localized in space long after journals were everywhere. The scarce input was the person who had read the paper and was also standing next to the problem, and that person is what the model replaces.

\paragraph{Adoption cost.} Once the match is made, the model can also write the implementation. The cost of taking a theorem from the page to a first implementation in an application falls from weeks of a specialist's time to minutes of anyone's, and \citet{brynjolfsson2025} find that the gain from such assistance is largest for the least experienced, which is to say for the people who were not going to be the bilingual person.

The firms that already had quants are the less important case. For them the cost was already low, and a further fall is worth a factor of a few. For the long tail of businesses with no quant, the cost was above any value on offer, nothing was adopted, and a cost that falls from prohibitive to affordable adds a term to the sum rather than multiplying one. Most business activity has never been written down as a control problem, an allocation, or a forecast, and that is where most uses are. Whether that tail is worth as much as the incumbents or ten times as much is a hypothesis, and the scenarios carry it as one.

\paragraph{Lag.} A result posted in the morning can be retrieved and read by a model in the afternoon. The lag from publication to availability at every use collapses from the decades the historical record shows to the same day. What remains is waiting for a problem the result fits. That wait belongs to the problem stream, not to transmission. At a three per cent discount rate, removing forty years of lag multiplies present value by about 3.3. At five per cent, removing fifty years gives about twelve.

The 68-year median is a field-level figure whose clock starts at the field's founding; the lag that matters starts when a particular result is published, and the two are different quantities. The record establishes that lags of decades were normal, and \citet{adams1990} finds the same scale in industrial productivity data, twenty to thirty years from academic science to measured effect. The year counts used in the scenarios are assumptions on that scale.
...
The value of a result is the number of uses it reaches, times the net value of a use, times a discount for the wait. The three channels multiply within a segment of uses and average across segments. Appendix A states the identity and a power-law planner, chosen to be transparent rather than realistic.

Appendix C sets the channels at mild, large, and extreme values, and the multiplier on gross application value that comes out is 8, 190, or 36{,}000. The multiplier on net value is that number times a screening adjustment the model does not estimate, and the multiplier on a marginal result is that number times a marginal-stock factor it does not estimate either; the table carries both as rows set to one. Every column points the same way because every channel is assumed to move the same way. The columns are three sizes of one conditional calculation, not a lower bound and an upper bound, and the size is illustrated, not measured.

\section{Optimal investment}

A planner spends $S$ dollars on unmotivated mathematics and obtains $I(S) = a S^{\gamma}$ results, $0 < \gamma < 1$. Each result has expected value $V$, the value of one more result given the results already reachable. The model also makes the existing stock of results reachable, which raises the value realized from that stock and may substitute for some new production; that stock effect is not part of $V$. The multipliers of Appendix A are computed for a typical result, and the scenarios assume they apply to the marginal result as well.

\begin{assumption}[Marginal transport]\label{ass:transport}
Write the multiplier on the expected application value of a marginal new result as $\lambda\kappa$, where $\kappa$ is the multiplier on a representative existing result and $\lambda$ is a marginal-stock factor that absorbs substitution from newly accessible existing mathematics. A new result enters the same matching process as an old one. The direction of the results below requires only that the net marginal multiplier $\mu_M = \lambda\kappa^G_M\sigma$ of Table \ref{tab:channels} exceed one, that substitution and screening together do not outweigh transport; the sizes in the table set $\lambda = 1$ and $\sigma = 1$.
\end{assumption}

With diminishing returns of the power-law kind, a planner who values each result more spends more than proportionately (Appendix B), so the ratio of spending on mathematics to spending on the professions rises by 8, 190, or 51{,}000 across the three scenarios, once the professions' own multiplier is allowed for and with the screening adjustment and $\lambda$ both at one; the ratio rises whenever the net marginal multiplier exceeds the professions' multiplier.

The multiplier for mathematics and the multiplier for the professions, $\kappa_L$, may move in opposite directions. The product of mathematics is a non-rival idea, and the model raises its reach. The product of a law or commerce degree is a person's capacity to perform tasks, and the model performs a share of those tasks itself. \citet{eloundou2023} rate legal services among the occupations most exposed to language models. Exposure is not substitution, so I take $\kappa_L = 1$ as the baseline and $\kappa_L < 1$ as the sensitivity.

The same first-order condition gives the underinvestment wedge in the same terms. Let $W$ be the social value of a result and $P$ the part of it a producer can appropriate, and let the model multiply them by $\kappa_W$ and $\kappa_P$.

\begin{assumption}[Weak appropriation]
The multiplier on social value exceeds the multiplier on appropriable value, $\kappa_W > \kappa_P$.
\end{assumption}

\begin{remark}[The wedge]
...
This is Nelson and Arrow restated for the mechanism at hand. The assumption is plausible because the new uses are unpriced and the author is not in the room, though reputation, grants, and consulting also scale with reach; under it, the wedge that public funding exists to close grows by the ratio. The case for funding pure mathematics rests on that wedge, and the ratio to the professions is an illustration of its size.

Ideas may also be getting harder to find. \citet{bloom2020} document falling research productivity across fields, and that is a statement about $a$ in \eqref{eq:foc}, the production side. The present argument is about $V$, the value side, and the two enter multiplicatively. A field can be harder to advance and, at the same time, far more valuable to advance.

The model may do the mathematics too. If it does, $a$ rises, and by \eqref{eq:foc} optimal spending rises with it, by the same elasticity as for a rise in $V$. The question is how much unmotivated mathematics society should want, and a cheaper production technology increases both the quantity and the optimal spend. Who or what produces it is a separate matter, and the human share of the pipeline is a question about labour markets rather than about welfare.

Or it may invent the mathematics on demand. If a system can produce the needed result when the application arises, the option value of producing it decades ahead falls.

Two things stand against this. Grossmann needed a library that already held Riemann, Ricci, and Levi-Civita; on-demand invention at the frontier is a harder task than on-demand retrieval, and unmotivated work is at the frontier. And a result invented on demand is invented at every use, whereas a result produced once is non-rival. Paying $K$ times for what could be paid for once is the opposite of what non-rivalry recommends. A result invented on demand and then cached has been produced once and is the same non-rival good, so the comparison is between the cost of producing it in advance and the cost of producing it when first needed, plus the value lost in the interval, and the intervals in the record run to decades.
...
The observed allocation is moving the other way in Australia and the United Kingdom. In both, the supply of mathematics is being cut, and in both the cuts are a response to university finances rather than to any estimate of the value of the output.
...
There is nothing wrong with the product. Pure mathematics is not only manifestly useful on the empirical record; it is the foundation of the modern world. The structures that map reality are not useful to engineering, policy, and commerce in the way a convenience is useful. They are vital.

The efficiency of abstraction and generalization is equally obvious, and it is most apparent when it fails: the same abstract idea travelling down parallel fields for decades before anyone notices the two are one. One two-line idea, push a variable through its own distribution function and pull a uniform back through an inverse, has been reinvented and renamed in goodness-of-fit testing, Monte Carlo, copulas, optimal transport, forecast evaluation, normalizing flows, and conformal prediction; \citet{cotton2026skaters} traces more than a hundred dated instances of it across a century.

The declaration's central distinction is between solving problems and understanding, and its fear is for the transmission chain from proof to textbook. Its one concession, that AI may enhance and accelerate genuine understanding, is the sentence to build on.

I offer a brighter reading of precisely that distinction. If understanding is the goal, cheaper access to solutions is a gift. A solved problem remains available to understand, simplify, generalise, connect to other fields, and teach. Knowing that a route exists makes exploring the landscape considerably more productive.
...
I tested this too. I gave two recent proofs, each over a hundred pages and by reputation hard going, Wang and Zahl's proof of the Kakeya conjecture in three dimensions \citep{wangzahl2025} and Baek's proof that Gerver's sofa is optimal \citep{baek2024}, to a fresh instance of the same model, with the instruction to extract one idea that a strong non-specialist could follow, that was new in the paper, with a toy case and a candidate use elsewhere.

From the first it returned the factoring proposition, the idea the paper itself names as new, with a toy version in the plane and a reading of it as a regularity lemma for point sets with a greedy algorithm attached. From the second it returned the injectivity condition, with the eleven-step iteration that proves it presented as an undergraduate exercise, and the observation that a piano-mover's obstacle corner traces the same curve. In both it separated what was new from what was inherited and said which sections it had read. That is the textbook end of the journey, and it took an hour.
...
The declaration does not argue for a slow chain, and the misalignment it describes and the undersupply argued here can both be true. Its problem is internal to mathematics, a matter of incentives, attribution, verification, and integration; the problem here is external, a transmission externality between mathematics and its uses. The chain it defends, kept as slow as it has been, would leave the second problem in place. A single phrase can unlock immediate value. A hint such as \emph{can the Reynolds projection simplify pandemic modelling} should not trickle through the system over decades if it can help now, and a model that has read both literatures can offer the hint the same afternoon.

The declaration values technique and the patience to turn insight into theorem, and it should. But technology moves on. There was a time when detailed knowledge of registers and buses was necessary to get useful work from a computer, and nobody now thinks the programmers of that era were the only real ones.

Democratizing the proof may at first appear a threat. For some of us it is liberating: it removes the drudgery of turning insight into rigor, and leaves the insight. Rigor sometimes discovers that the insight was wrong, and a machine that supplies the rigor supplies that discovery too.

Rigor means two things. Rigor in the ordinary sense, checking that an argument holds, should not be a luxury good, and a model that supplies it on demand makes it cheap for anyone with an insight to check. Non-trivial rigor, inventing the argument a hard theorem needs, is another matter and stays hard. The declaration defends the second, and rightly. The first is what most uses of mathematics need, and it need not be scarce.
...
Pure mathematics is a language, and a language should not be scarce. Arguments sit on a continuum from a vague opinion to a theorem, and moving one along it used to cost a collaborator's month and now costs an afternoon. Yet the exchange of mathematical arguments between people who are not professional mathematicians remains rare, and the exchange between professionals moves at the pace of academic publishing, which is glacial. Perhaps one day it will be the norm in online forums to throw proofs and models back and forth the way code is thrown back and forth now, with a model doing the translation on each side.

The same holds for the humbler task of finding out what is already known. A recent paper of mine on inverting contests with correlated performances \citep{cotton2026contests} went through some twenty-five rounds of model-assisted literature search before it was posted, and the repository records what they found.

The inversion had been posed as a convex program by Li in 2018. The shared-field assembly at the heart of the algorithm was in Lambert's 1975 fault-tree thesis at Livermore, had been rediscovered in statistical timing analysis for chips between 2006 and 2012, and was Algorithm 4 of a 2016 bandits paper by Russo. The deletion ensemble was Chiang's partial crude probabilities of 1961, with Makeham 1874 underneath, and identification was Anderson and Ghurye 1977. None of these was cited in the 2021 version of the work.

An adjacent note lost its novelty claim entirely, to an exact identity in a workshop paper. Corrections to this one are welcome at \texttt{github.com/microprediction/pure}.
...
Under the assumptions stated, this need has risen by a multiple. Whether that multiple is 8, 190, or 51{,}000 is a question about the channel sizes, and the columns of Table \ref{tab:channels} disagree about it by four orders of magnitude. Under the assumptions stated they do not disagree about the direction.
...
\appendix
\section{The value of a result}
\label{app:value}

The three appendices are the minimal talking point behind the multipliers in the text. Nothing in them is more than bookkeeping, and the numbers they produce are illustrations.

Fix a result $r$ produced by unmotivated research. Index its potential uses by $i = 1, \dots, K$. Use $i$ has value $v_i > 0$ if the result is applied to it, measured as the gain over the best method available without it. Adopting it costs $c_i > 0$: the work of understanding and implementing the result once the match is made. $A_i \in \{0, 1\}$ indicates whether the match is made, and if it is, it happens after a lag $T_i \ge 0$. Search cost determines $A_i$ and is not charged in $c_i$. The result is non-rival: use $i$ does not diminish use $j$. With social discount rate $\rho$, the realized social value is
\begin{equation}
...
\label{eq:closed}
\end{equation}
$D$ is the expectation of $e^{-\rho T}$, which by Jensen's inequality is at least $e^{-\rho\, \E T}$. Write $D = e^{-\rho \tilde\tau}$; then $\tilde\tau = -\rho^{-1} \log D$ is the \emph{certainty-equivalent lag}, and the lag figures below are in these units.

The elasticities are immediate. Value is linear in the encounter probability, has elasticity $-(\alpha - 1)$ in adoption cost on the domain $c \ge v_0$, and falls at rate $\rho$ per year of certainty-equivalent lag.
...
Equation \eqref{eq:closed} makes two assumptions. First, the value of a use is independent of whether and when it is reached. Where that fails, replace $q$ by the value-weighted encounter probability $\tilde q = \E[(v-c)^+ A]\,/\,\E[(v-c)^+]$; and $D$ by $\tilde D = \E[(v-c)^+ A e^{-\rho T}]\,/\,\E[(v-c)^+ A]$, which allows value and timing to be dependent; everything below goes through with $\tilde q$ and $\tilde D$ in place of $q$ and $D$, and the reach multiplier is then a ratio of value-weighted probabilities. With a heavy tail the two can differ a great deal: if the old mechanism had found exactly the top decile of uses under the central Pareto parameters, universal reach would multiply value by about 1.5, not ten. Whether the old allocation, which found the uses at firms that could afford a mathematician, approximated selection on application value is an empirical question; the scenarios assume that selection was weak.

Second, reach and lag come from one matching process. Reach means matched within a fixed horizon, $D$ is the expected discount given a match, and with those definitions the product form is exact.

Matches that are encountered and rejected cost something to verify, and \eqref{eq:value} omits that cost. Write $C(q)$ for the expected present-value cost of screening candidates, false ones included, at reach $q$; the expected value is then $K q\, g(c) D - C(q)$, and raising reach raises welfare only where the gain in gross application value exceeds the rise in $C$. Writing $G_t = K q_t g(c_t) D_t$ for gross value and $s_t = C_t / G_t$ for the screening share, net value is $G_t (1 - s_t)$ and its ratio across regimes is $(G_1/G_0)\,(1-s_1)/(1-s_0)$. The channel multipliers in the scenarios are $G_1/G_0$, gross of screening. The second factor needs a primitive for false candidates that the model does not carry, and Table \ref{tab:channels} carries it as a separate row set to one. Cheap screening does not by itself put it at one: a model makes screening cheap, which pushes the factor above one, and makes false candidates common, which pushes it below, and the row records the assumption that the two cancel. Before a model, screening was the main reason encounters were rare.

Before large language models, a use could encounter a result only if a person at that use was bilingual, fluent in the problem and fluent enough in the mathematics to recognize the match. The encounter probability of an unmotivated result was the probability that such a person was present, and \citet{burt2004} measured how rare such people are and how much of the supply of good ideas passes through them.

Marcel Grossmann is the canonical case. In 1912 Einstein needed a geometry of curved space and did not have one; Grossmann, his friend from the Zurich Polytechnic, went to the library, researched the problem, and realized that the purely theoretical work of Riemann, Ricci, and Levi-Civita was exactly what Einstein needed \citep{pais1982}. General relativity had a bilingual person on hand. Most problems do not, and the nine reinventions above are the record of it.

\subsection*{The channels}
...
\label{eq:segments}
\end{equation}
a weighted average of the two segment multipliers with pre-LLM value as the weights.

The channels multiply within a segment and average across segments. A small $\varepsilon$ and a large $\delta$ is the case in which the old tail was small and the new tail rivals the incumbents. In the long tail $\delta$ may be large. Table \ref{tab:channels} carries the ratio $\kappa / \kappa_Q = (1+\delta)/(1+\varepsilon)$ as a separate row. The segmentation is a hypothesis about where adoption costs fell, not a measurement.

The lag multiplier is $D_1 / D_0 = e^{\rho(\tilde\tau_0 - \tilde\tau_1)}$; at three per cent, removing forty years of certainty-equivalent lag gives $e^{0.03 \times 40} \approx 3.3$, and at five per cent, removing fifty years gives about twelve.
...
\begin{remark}[The multiplier is largest where the old matching was worst]
...
This is immediate from \eqref{eq:kappa}, since $n = q_1/q_0$, the lag factor is $D_1 / D_0$, and \eqref{eq:segments} is a weighted average with positive weights.

\section{The planner's problem}

The planner of the main text obtains $I(S) = a S^{\gamma}$ results from spending $S$, $0 < \gamma < 1$, each of expected value $V$. The planner maximizes $a V S^{\gamma} - S$. The first-order condition $\gamma a V S^{\gamma-1} = 1$ gives
\begin{equation}
...
\begin{remark}[Spending elasticity]
...
Why does the elasticity exceed one? With diminishing returns the marginal result is cheap when spending is low and expensive when spending is high. If each result becomes ten times more valuable, the planner keeps buying until the marginal result costs ten times more, and under a square-root ideas function that means spending one hundred times more.

This is a property of the power law, not of research spending in general: for any $I(S)$ the local elasticity is $1/\eta(S^\star)$ with $\eta = -\,d\log I'(S)/d\log S$, and the power law is the case of constant $\eta = 1-\gamma$. Extrapolating a constant $\gamma$ across orders of magnitude is not defensible, so the scenarios use the limit $\gamma \to 0$, where the planner's response is proportional to value. That is the smallest elasticity within the power-law family, not a floor in general: a saturating $I(S) = 1 - e^{-S}$ gives $S^\star = \log V$ and an elasticity of $1/\log V$, below one for $V > e$.
...
\subsection*{The ratio to law and commerce}
...
\label{eq:ratio}
\end{equation}
This is the same elasticity as \eqref{eq:foc}. If a language model multiplies the net value of a marginal result in mathematics by $\mu_M$ and $V_L$ by $\kappa_L$, the ratio is multiplied by $(\mu_M / \kappa_L)^{1/(1-\gamma)}$; the ratio rises when $\mu_M > \kappa_L$; a change in either sector's productivity $a$ enters the same way, and the scenarios hold both fixed.

\section{Scenarios}

Table \ref{tab:channels} sets out mild, large, and extreme values for each channel and the change in the benchmark ratio they imply through \eqref{eq:ratio}. The columns are three sizes of one conditional calculation, not a calibration, and the mild column is not a lower bound: it still assumes reach triples, adoption value rises, twenty-five years of lag disappear, and the screening and marginal-stock factors sit at one.

The first three rows are the incumbent segment. Its reach multiplier $n_Q$ is the ratio of the post-LLM to the pre-LLM encounter probability there; one worker in ten is the share \citet{deloitte2013} count as mathematically occupied across the economy, and one problem in thirty is a guess at how often such a worker was actually in the room, and since quant firms had such workers more often than the economy did, these values are if anything generous for $Q$.

The adoption-cost multiplier uses $m_Q = 3$, $10$, and $10$ with tail indices $1.2$, $1.5$, and $2$, on the domain $c_Q \ge m_Q v_0$. The lag multiplier uses discount rates of three and five per cent and reductions in certainty-equivalent lag of twenty-five, forty, and fifty years, and even the first of these doubles present value on its own; the last is short of the 68-year median because part of every historical lag was waiting for the problem to exist.

The tail row is $(1+\delta)/(1+\varepsilon)$ from \eqref{eq:segments} with $\varepsilon = 0.1$ throughout: the mild column has $\delta = \varepsilon$, a tail no better than the incumbents; the large column has $\delta = 1.1$, a new tail slightly larger than the incumbents; the extreme column has $\delta = 10$. Channel entries are rounded before multiplying.

\begin{table}[tbp]
\centering
\begin{tabular}{lccc}
\toprule
Channel & Mild & Large & Extreme \\
\midrule
Reach $n_Q$ & 3 & 10 & 30 \\
...
\midrule
Gross value multiplier $\kappa_M^G$ & 7.6 & 190 & 36000 \\
Screening adjustment $\sigma = (1-s_1)/(1-s_0)$, assumed & 1 & 1 & 1 \\
Marginal-stock factor $\lambda$, assumed & 1 & 1 & 1 \\
Net marginal multiplier $\mu_M = \lambda\kappa_M^G\sigma$ & 7.6 & 190 & 36000 \\
Professional multiplier $\kappa_L$ & 1.0 & 1.0 & 0.7 \\
\midrule
Ratio multiplier, elasticity one & 7.6 & 190 & 51000 \\
\bottomrule
\end{tabular}
\caption{Channel multipliers and the implied change in the ratio of spending on unmotivated mathematics to spending on the professions, from \eqref{eq:ratio}. The three channels multiply within the incumbent segment and the tail enters by the weighted average \eqref{eq:segments}. The two assumed rows are the screening adjustment of Appendix A and the marginal-stock factor of Assumption \ref{ass:transport}; the net marginal multiplier $\mu_M$ is the product of the three rows above it and is what enters \eqref{eq:ratio} in place of $\kappa_M$. Optimal spending on mathematics rises when $\mu_M > 1$, and the ratio rises when $\mu_M > \kappa_L$. The ratio row uses elasticity one, the limit $\gamma \to 0$ of \eqref{eq:ratio}.}
\label{tab:channels}
\end{table}

The extreme column is not a forecast; it shows the range once the channels compound. The mild column, with a reach multiplier of three, a weak cost channel, and the smallest lag improvement, moves the ratio by a factor of nearly eight. The large column moves it by two orders of magnitude at elasticity one; within the power-law family any $\gamma$ above zero moves it further. The reach multiplier is a ratio, so a value near one says nothing on its own about how often a bilingual person was present before. If the post-LLM encounter probability is near one, a reach multiplier near one would imply that a suitable bilingual person was already present at most uses, and the case studies say otherwise.
...
\bibitem[Baek(2024)]{baek2024}
Baek, J. (2024).
\newblock Optimality of Gerver's sofa.
\newblock arXiv:2411.19826.
...
\bibitem[Cotton(2026c)]{cotton2026contests}
Cotton, P. (2026c).
\newblock Scalable inversion of contests with correlated performances, including softmax and multinomial probit.
\newblock arXiv:2609.01133.

\bibitem[Cotton(2026d)]{cotton2026skaters}
Cotton, P. (2026d).
