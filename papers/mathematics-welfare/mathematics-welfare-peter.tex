...
Fix a result $r$ produced by unmotivated research. Index its potential uses by $i = 1, \dots, K$. Use $i$ has gross value $v_i > 0$ if the result is applied to it. Adopting it costs $c_i > 0$: the work of understanding and implementing the result once the match is made. $A_i \in \{0, 1\}$ indicates whether the match is made, and if it is, it happens after a lag $T_i \ge 0$. Search cost determines $A_i$ and is not charged in $c_i$. Matches that are encountered and rejected cost something to verify, and \eqref{eq:value} omits that cost: with a model it is minutes, and before a model it was the main reason encounters were rare. The result is non-rival: use $i$ does not diminish use $j$. With social discount rate $\rho$, the realized social value is
\begin{equation}
...
\label{eq:closed}
\end{equation}
$D$ is the expectation of $e^{-\rho T}$, which by Jensen's inequality exceeds $e^{-\rho\, \E T}$. Write $D = e^{-\rho \tilde\tau}$; then $\tilde\tau = -\rho^{-1} \log D$ is the \emph{certainty-equivalent lag}, and the lag figures below are in these units. The elasticities are immediate. Value is linear in the encounter probability, has elasticity $-(\alpha - 1)$ in adoption cost on the domain $c \ge v_0$, and falls at rate $\rho$ per year of certainty-equivalent lag.
...
Equation \eqref{eq:closed} makes two assumptions. First, the value of a use is independent of whether and when it is reached. This is generous to the argument, since a use valuable enough to hire a mathematician was more likely to be reached under the old regime. Second, reach and lag come from one matching process. Reach means matched within a fixed horizon, $D$ is the expected discount given a match, and with those definitions the product form is exact.
