...
\begin{enumerate}
\item Transmission was the bottleneck, not the mathematics. Every named dismissal of a pure field on record has failed, the median wait from a field's founding to first external use was 68 years, and nine documented reinventions show why: a result reached a use only when someone in the room knew both the problem and the theorem, a bilingual person.
...
\item So the marginal result is worth a multiple, and the gap between what society should spend and what private returns will pay for has widened, since the new uses are unpriced. The multiple is illustrated, not measured; the direction is not in doubt.
...
\paragraph{One invariant, four names.} Given a matrix $X$, matroid theory defined the girth of its vector matroid, compressed sensing defined its spark, tensor decomposition defined its Kruskal rank, and coding theory defined its $k$-stability. The identities are exact,
\begin{equation}
...
\paragraph{Factor graphs.} Coding theory had the BCJR algorithm and turbo decoding, statistics had the forward-backward algorithm and the Kalman filter, and artificial intelligence had belief propagation. All are the same operation, factor a global function into local ones and pass messages that eliminate variables, and the BCJR algorithm was reinvented several times before anyone said so. The factor-graph formulation then made receiver architectures derivable that no one had composed.
...
\begin{tikzpicture}[x=1cm, y=1cm, >=Stealth, font=\footnotesize]
  ...
  \fill (8.68,3.00) circle (1.6pt); \node[above,yshift=20pt] at (8.68,3.00) {girth NP-hard 1997};
  \draw[dashed,->] (8.68,3.00) -- (8.68,1.03);
  \draw[very thick] (8.68,0.88) -- (10.51,0.88); \fill (10.51,0.88) circle (1.6pt);
  \node[below,yshift=-9pt] at (10.51,0.88) {spark 2014};
...
This need has risen by a multiple. Whether that multiple is 8, 190, or 51{,}000 is a question about the channel sizes, and the columns of Table \ref{tab:channels} disagree about it by four orders of magnitude. They do not disagree about the direction.
...
\label{eq:ratio}
\end{equation}
This is the same elasticity as \eqref{eq:foc}. If a language model multiplies $V_M$ by $\kappa_M$ and $V_L$ by $\kappa_L$, the ratio is multiplied by $(\kappa_M / \kappa_L)^{1/(1-\gamma)}$.
