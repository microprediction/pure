...
\begin{abstract}
Pure mathematics (and applied mathematics pursued without a client) produce ideas whose social value depends on how many uses they reach, what each use costs to make, and how long it takes before somebody matches the tool to the application. We examine the multiplicative impact of large language models on all three in a highly stylized model for reach, adoption cost, and lag. Whatever view one might have on current spending, the implication is dramatic and, notwithstanding the difficulty of estimating terms or the imperfection in this simplistic analysis, the conclusion is unmistakable: society is massively underinvesting in pure research. We place this observation in the context of recent cutbacks to funding, and the recently protested alignment crisis between AI and pure mathematics.
\end{abstract}
...
At the time, I recall being persuaded against further pursuit of mathematics. The professors who taught me bemoaned what they felt was a gutting of the profession - something roughly equivalent to a halving of real income over a generation. I did pursue a doctorate, as it happens, but neither myself nor anyone else from that small class went on to work on what I will term ``unmotivated mathematics''. Australia is not necessarily representative, but I suspect not entirely unusual either.

I spent the next three decades as an applied mathematician, albeit one with a love and respect for pure mathematics. I have witnessed on countless occasions the failure of businesses to recognize the context: the abstract structure of their operations: the market-maker who does not realize they are solving a control problem; the data teams who perform unwitting filtering; the countless ``data scientists'' who are comfortable with that ridiculous phrase mocking the existence of the statistical profession; the CDO modelers who reinvented Copulas without noticing; the trader who intuitively uses shrinkage but rarely optimally; the epidemiologist who assumes a flat term structure. The default is to {\em never} formalize.

The lesser sin is renaming. Backpropagation, gaussian process regression, softmax classifiers, Kalman filtering, attention, message passing, random features, contrastive learning, learning to rank, synthetic controls, value iteration, mirror descent, ensemble uncertainty ... none of these are truly original mathematical structures. All had almost identical predecessors. Finding and inventing new structure is difficult. Working at the highest level of abstraction is physically hard. Too hard for me, as it happens. I moved to applied work early on in my Ph.D.

\section{Unmotivated mathematics}

It is my impression however, that when mathematical progress is made it almost always finds application. And conversely, it is hard to think of a major industry that does not stand upon its foundations. It is intuitively efficient from the perspective of society at large for those with the ability and drive, to perform focussed work that accumulates this bedrock over centuries and not wait until it might be useful. By that time it is too late and the odds that the person who needs it, wittingly or otherwise, would also have the awareness, talent and time to solve the problem on the fly is remote. That would be like hoping that Einstein could invent differential geometry because it was needed. He did not, and probably was not capable of doing so.
...
The question is what the ratio of investment in mathematics to investment in the professions should be, and how it changes now that large language models exist.
...
Fix a result $r$ produced by unmotivated research. Index its potential uses by $i = 1, \dots, K$. Use $i$ has gross value $v_i > 0$ if the result is applied to it, an adoption cost $c_i > 0$ covering the work, after the match is made, of understanding and implementing the result in that context, and, if the match is ever made, a lag $T_i \ge 0$ before it happens. Whether the match is made at all is the encounter indicator $A_i \in \{0, 1\}$; the cost of searching for the match is what determines $A_i$ and is not charged again in $c_i$. Encountered matches that are then rejected incur a verification cost that \eqref{eq:value} omits; with a model it is minutes, and before a model it was the reason encounters were rare rather than a cost paid at every use. The result is non-rival: use $i$ does not diminish use $j$. With social discount rate $\rho$, the realized social value is
\begin{equation}
V(r) \;=\; \sum_{i=1}^{K} A_i\,(v_i - c_i)^{+}\,e^{-\rho T_i}.
\label{eq:value}
\end{equation}
In English: add up every use the result ever reaches, net of what each use costs to adopt, and discount each by how long it took. Four primitives enter: the number of potential uses $K$, the encounter indicators $A_i$, the adoption costs $c_i$, and the lags $T_i$.
...
A large language model is, for present purposes, a device that makes every result in its training corpus or retrieval index available at every use simultaneously.
...
The common cost $c$ in \eqref{eq:closed} hides the important case, so split the uses into two segments. Segment $Q$ is the few large firms with teams of quants, for which $c_Q$ was already low; by \eqref{eq:closed} a cost reduction by factor $m$ multiplies its value by $m^{\alpha-1}$, provided $c_Q/m$ still exceeds $v_0$, and with $\alpha$ near one that is a factor of a few. Segment $H$ is the long tail of businesses with no quant, for which $c_H$ sat far above the values on offer; since $g(c) \propto c^{1-\alpha}$, its pre-LLM contribution $W_{H0}$ was not zero but negligible. A cost that falls from prohibitive to affordable does not multiply $W_{H0}$; it adds a term.

Write $W_{jt} = K_j\, q_{jt}\, g(c_{jt})\, D_{jt}$ for the expected value from segment $j$ in regime $t$, each segment with its own primitives, so that $W_t = W_{Qt} + W_{Ht}$. Let $\kappa_j = W_{j1}/W_{j0}$ be each segment's own multiplier and $\varepsilon = W_{H0}/W_{Q0}$ the tail's share of pre-LLM value relative to the incumbents. Then, exactly,
\begin{equation}
\kappa \;=\; \frac{W_{Q1} + W_{H1}}{W_{Q0} + W_{H0}} \;=\; \frac{\kappa_Q + \varepsilon\,\kappa_H}{1 + \varepsilon},
\label{eq:segments}
\end{equation}
a weighted average of the two segment multipliers with pre-LLM value as the weights. The channels multiply within a segment and average across segments. Since the tail's cost fell further and its encounter probability rose more, $\kappa_H \ge \kappa_Q$, and so $\kappa$ lies between $\kappa_Q$ and $\kappa_H$ wherever $\varepsilon$ sits; a large $\varepsilon$ moves $\kappa$ toward $\kappa_H$, not toward zero. Most business activity has never been written down as a control problem, an allocation, or a forecast, and that is where the count of uses lives, so $\kappa_H / \kappa_Q$ may be large. Table \ref{tab:channels} carries the ratio $\kappa / \kappa_Q$ as a separate row. The segmentation is a hypothesis about where adoption costs fell, not a measurement.
...
Why does the elasticity exceed one? With diminishing returns the marginal result is cheap when spending is low and expensive when spending is high. If each result becomes ten times more valuable, the planner keeps buying until the marginal result costs ten times more, and under a square-root ideas function that means spending one hundred times more. The smaller reading is the limit $\gamma \to 0$, where results barely respond to spending and the planner's response is proportional to value. We report both ends.
...
Lag $D_{Q1}/D_{Q0}$ & 2.1 & 3.3 & 30 \\
\midrule
Incumbent multiplier $\kappa_Q$ & 7.6 & 99 & 9000 \\
Tail averaging $\kappa/\kappa_Q$ & 1 & 2 & 10 \\
\midrule
Value multiplier $\kappa_M$ & 7.6 & 200 & 90000 \\
Professional multiplier $\kappa_L$ & 1.0 & 1.0 & 0.7 \\
\midrule
Ratio multiplier, $\gamma \to 0$ & 7.6 & 200 & 130000 \\
Ratio multiplier, $\gamma = \tfrac12$ & 58 & 40000 & $1.7 \times 10^{10}$ \\
\bottomrule
\end{tabular}
\caption{Channel multipliers and the implied change in the ratio of spending on unmotivated mathematics to spending on the professions, from \eqref{eq:ratio}. The three channels multiply within the incumbent segment and the tail enters by the weighted average \eqref{eq:segments}. The elasticity is $1/(1-\gamma)$: one in the limit $\gamma \to 0$, two at $\gamma = \tfrac12$.}
\label{tab:channels}
\end{table}

The high column is not a forecast. It is there to show the range once the channels compound. The low column, with a reach multiplier of three, a nearly useless cost channel, and a modest lag improvement, moves the ratio by a factor of nearly eight. The central column moves it by two orders of magnitude at elasticity one, and by more than four at $\gamma = \tfrac12$. A reach multiplier near one would mean a bilingual person was already present at most uses, and the case studies say otherwise.

Equation \eqref{eq:ratio} compares two planner optima and the observed allocation was not one, so the figures below normalize the observed ratio rather than locate the post-LLM optimum. Scaling the American degree counts by the same multipliers, with a degree in each sector as a unit of investment, the pre-LLM ratio of mathematics doctorates to law and business degrees combined was about one to 160. Under the low column it becomes one to 20; under the central column, at elasticity one, mathematics doctorates would outnumber law and business degrees. On a campus, six students against several thousand becomes fifty under the low column and a thousand under the central one. Degree counts are proxies for spending, not measures of it.
...
\paragraph{The ideas were already free.} Journals were open, or nearly so, for decades before language models, and the lags were still measured in generations. Free to read is not free to find, and not free to recognize as relevant; \citet{jaffe1993} found spillovers localized in space long after journals were everywhere. The scarce input was the person who had read the paper and was also standing next to the problem, and that person is what the model replaces.
...
There were six of us in the pure mathematics class at the University of New South Wales, and none of us became pure mathematicians. Six was still too few on the record as it already stood, before language models: every named dismissal of a pure field in the survey has failed, and the underinvestment argument is sixty years old. There should have been a hundred.
