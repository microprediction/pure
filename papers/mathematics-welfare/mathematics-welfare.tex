...
Write $W_{jt}$ for the expected value from segment $j$ in regime $t$, so that $W_t = W_{Qt} + W_{Ht}$. With $W_{H0} \approx 0$,
\begin{equation}
\kappa \;=\; \frac{W_{Q1} + W_{H1}}{W_{Q0}} \;=\; \kappa_Q\,(1 + \theta),
\qquad
\kappa_Q = \frac{W_{Q1}}{W_{Q0}},
\qquad
\theta = \frac{W_{H1}}{W_{Q1}},
\label{eq:segments}
\end{equation}
where $\theta$ is the post-LLM value of the tail relative to the post-LLM value of the incumbents. The channels multiply within a segment and add across segments. Most business activity has never been written down as a control problem, an allocation, or a forecast, and that is where the count of uses lives, so $\theta$ may be large; it is a separate parameter and Table \ref{tab:channels} carries it as one. The segmentation is a hypothesis about where adoption costs fell, not a measurement.
...
Collecting the three channels for the incumbent segment, and adding the tail by \eqref{eq:segments}, the ratio of post to pre expected value is
\begin{equation}
\kappa_Q \;=\; n \cdot m^{\alpha - 1} \cdot \frac{D_1}{D_0} \;=\; n \cdot m^{\alpha - 1} \cdot e^{\rho(\tilde\tau_0 - \tilde\tau_1)},
\qquad
\kappa \;=\; \kappa_Q\,(1 + \theta).
\label{eq:kappa}
\end{equation}

\begin{proposition}[The multiplier is largest where the old matching was worst]
Hold $m$, $\alpha$, $\rho$, $\theta$, $q_1$, and $D_1$ fixed. Then $\kappa$ is decreasing in the pre-LLM encounter probability $q_0$ and decreasing in the pre-LLM discount factor $D_0$, that is, increasing in the pre-LLM certainty-equivalent lag $\tilde\tau_0$. For research with a single designed-in user who possessed it at the outset, $q_0 = q_1 = 1$, $D_0 = D_1$, and $\theta = 0$, so $\kappa = m^{\alpha-1}$: only the adoption-cost channel remains.
\end{proposition}

\begin{proof}
Immediate from \eqref{eq:kappa}, since $n = q_1/q_0$, the lag factor is $D_1 / D_0$, and $1+\theta$ is a positive constant.
\end{proof}
...
The posterior is the benchmark ratio updated by the multipliers in \eqref{eq:ratio}. Table \ref{tab:channels} sets out a low, central, and high value for each channel and reads off the result. The reach multiplier $n$ is the ratio of the post-LLM to the pre-LLM encounter probability; one worker in ten is the share \citet{deloitte2013} count as mathematically occupied, and one problem in thirty is a guess at how often such a worker was actually in the room. The adoption-cost multiplier is for the incumbent segment: $m = 3$, $10$, and $10$ with tail indices $1.2$, $1.5$, and $2$, on the domain $c_Q \ge m v_0$. The tail entry $\theta$ is set to zero, one, and nine: no tail, a tail worth as much as the incumbents, and a tail worth nine times as much. The lag multiplier uses discount rates of three and five per cent and reductions in certainty-equivalent lag of twenty-five, forty, and sixty-eight years. Channel entries are rounded before multiplying.
...
\bottomrule
\end{tabular}
\caption{Channel multipliers and the implied change in the ratio of spending on unmotivated mathematics to spending on the professions, from \eqref{eq:ratio}. The three channels multiply within the incumbent segment and the tail adds by \eqref{eq:segments}. The elasticity is $1/(1-\gamma)$: one in the limit $\gamma \to 0$, two at $\gamma = \tfrac12$.}
\label{tab:channels}
\end{table}

The high column is not a forecast. It is there to show the range once the channels compound. The low column, with a reach multiplier of three, a nearly useless cost channel, and a modest lag improvement, moves the ratio by a factor of nearly eight. The central column moves it by two orders of magnitude at the conservative elasticity, and by more than four at $\gamma = \tfrac12$. A reach multiplier near one would mean a bilingual person was already present at most uses, and the case studies say otherwise.

Scaling the American degree counts of Section 1 by the same multipliers, with a degree in each sector as a unit of investment, the pre-LLM ratio of mathematics doctorates to law and business degrees combined was about one to 160. Under the low column it becomes one to 20; under the central column, at the conservative elasticity, mathematics doctorates would outnumber law and business degrees. On a campus, six students against several thousand becomes fifty under the low column and a thousand under the central one. Degree counts are proxies for spending, not measures of it.
