...
Equation \eqref{eq:ratio} compares two planner optima, and the observed allocation was not one, so what follows is a normalization of the observed ratio, not the post-LLM optimum. Scaling the American degree counts of Section 1 by the same multipliers, with a degree in each sector as a unit of investment, the pre-LLM ratio of mathematics doctorates to law and business degrees combined was about one to 160. Under the low column it becomes one to 20; under the central column, at elasticity one, mathematics doctorates would outnumber law and business degrees. On a campus, six students against several thousand becomes fifty under the low column and a thousand under the central one. Degree counts are proxies for spending, not measures of it.
...
This paper is an editorial with a model attached, and the model is not a result. Arguments sit on a continuum from a vague opinion to a theorem, and this one began at the vague end: a class of six felt too small. Moving an argument along that continuum used to cost a collaborator's month and now costs an afternoon, and moving people along it cannot hurt. The reader who wants to move this one further is welcome to point a model at the repository in which it lives, \texttt{github.com/microprediction/pure}, and say what is wrong.
