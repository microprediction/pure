...
\paragraph{Lag.} A result posted in the morning can be retrieved and read by a model in the afternoon. The lag from publication to availability at every use collapses from the decades the historical record shows to the same day. What remains is the wait for a problem the result fits to arise, and that is a property of the problem stream, not of transmission. At a three per cent discount rate, removing a forty-year lag multiplies present value by $e^{0.03 \times 40} \approx 3.3$. At five per cent, and from the survey median of 68 years, the multiplier is about thirty.

The 68-year figure needs a caveat. It is a field-level statistic, from a field's founding to the first documented external application of anything in it, and it is not the same object as the lag $T_i$ in \eqref{eq:value}, which attaches to one result and one use. A field's first application is the minimum over many results and uses, so the field-level median understates the result-level typical lag rather than overstating it. We use the figure as a scale, not a measurement.

Collecting the three channels, the ratio of post to pre expected value is
\begin{equation}
\kappa \;=\; n \cdot m^{\alpha - 1} \cdot e^{\rho(\tau_0 - \tau_1)}.
\label{eq:kappa}
\end{equation}

\begin{proposition}[The multiplier is largest where the old matching was worst]
Hold $m$, $\alpha$, $\rho$, and $\tau_1$ fixed. Then $\kappa$ is decreasing in the pre-LLM encounter probability $q_0$ and increasing in the pre-LLM lag $\tau_0$. For research with a single designed-in user who possessed it at the outset, $q_0 = q_1 = 1$ and $\tau_0 = \tau_1$, so $\kappa = m^{\alpha-1}$: only the adoption-cost channel remains.
\end{proposition}

\begin{proof}
Immediate from \eqref{eq:kappa}, since $n = q_1/q_0$ and the lag factor is $e^{\rho \tau_0}$ up to a constant.
\end{proof}
...
Table \ref{tab:channels} sets out a low, central, and high value for each channel. The reach multiplier $n$ is the ratio of the post-LLM to the pre-LLM encounter probability; the values are assumptions, loosely motivated by the roughly one worker in ten that \citet{deloitte2013} count as mathematically occupied, though that share is a different denominator from the share of problems at which such a worker was actually consulted. The adoption-cost multiplier uses $m = 3$, $10$, and $10$ with tail indices $1.2$, $1.5$, and $2$, and requires $c \ge m v_0$. The lag multiplier uses discount rates of three and five per cent and lag reductions of twenty-five, forty, and sixty-eight years, the transmission lag itself being gone. The channel entries are rounded before multiplying.
...
Lag $e^{\rho(\tau_0 - \tau_1)}$ & 2.1 & 3.3 & 30 \\
\midrule
...
Applied to the American degree counts in Section 1, and treating a degree in each sector as a unit of investment of comparable cost, the pre-LLM ratio of mathematics doctorates to law and business degrees combined was about one to 160. The low posterior is one to 20. The central posterior, at the conservative elasticity, is about one to two. Applied to a campus, six students against several thousand becomes fifty against several thousand in the low case, and a lecture theatre in the central one. The degree counts are proxies: they are not commensurate measures of spending, and the conversion assumes proportional unit costs.
...
The declaration describes the long journey from a difficult proof to a clear textbook explanation. Why assume the machine's contribution will be concentrated at the first end of that journey? Finding illuminating special cases, testing which hypotheses are necessary, suggesting alternative proofs, and translating between mathematical languages are all worthwhile targets, and they are exactly the activities that the historical record shows were undersupplied. The chain the declaration describes is internal to mathematics, from proof to textbook, and the 68-year figure measures something external, from field to application. The two are in series: a result that has not yet reached a textbook has rarely reached an engineer. It was never the mathematics that took a lifetime to become useful. It was the translation, at both stages.
