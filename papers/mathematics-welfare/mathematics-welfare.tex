...
The common cost $c$ in \eqref{eq:closed} hides the important case. For the few large firms with teams of quants, $c$ was already low, and by \eqref{eq:closed} the gain from a cost reduction by factor $m$ is $m^{\alpha-1}$, provided $c/m$ still exceeds $v_0$. With $\alpha$ near one that is a factor of a few, because value in that segment is dominated by the enormous uses that would have paid the old cost anyway. For the long tail of businesses with no quant, $c_i$ sat above every $v_i$ they had, nothing was adopted, and the segment contributed nothing to the pre-LLM sum at all. A cost that falls from prohibitive to trivial is not a multiplier on that segment; it is the segment entering the sum for the first time.

Most business activity has never been written down as a control problem, an allocation, or a forecast, and that is where the count of uses lives. In \eqref{eq:value} a use whose adoption cost exceeds every value it could have is indistinguishable from a use never encountered, so this tail belongs to the reach multiplier $n$, not to $m^{\alpha-1}$. The cost row of Table \ref{tab:channels} is the large-firm segment only; the reach row carries the rest.
...
The posterior is the benchmark ratio updated by the multipliers in \eqref{eq:ratio}. Table \ref{tab:channels} sets out a low, central, and high value for each channel and reads off the result. The reach multiplier $n$ is the ratio of the post-LLM to the pre-LLM encounter probability; one worker in ten is the share \citet{deloitte2013} count as mathematically occupied, one problem in thirty is a guess at how often such a worker was actually in the room, and the long tail of businesses where nobody was in the room, and where adoption cost alone kept every use out of the sum, sits in this multiplier too. The adoption-cost multiplier is for the segment that could already pay: $m = 3$, $10$, and $10$ with tail indices $1.2$, $1.5$, and $2$, on the domain $c \ge m v_0$. The lag multiplier uses discount rates of three and five per cent and reductions in certainty-equivalent lag of twenty-five, forty, and sixty-eight years. Channel entries are rounded before multiplying.
