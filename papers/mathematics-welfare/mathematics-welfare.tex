...
\begin{enumerate}
\item Transmission was the bottleneck, not the mathematics. All eight named dismissals of a pure field found in the survey have failed, the median wait from a field's founding to first external use was 68 years across 33 dated fields, and nine documented reinventions show why: a result reached a use only when someone along the way knew both the problem and the theorem, a bilingual person.
...
\label{eq:ratio}
\end{equation}
This is the same elasticity as \eqref{eq:foc}. If a language model multiplies $V_M$ by $\kappa_M$ and $V_L$ by $\kappa_L$, the ratio is multiplied by $(\kappa_M / \kappa_L)^{1/(1-\gamma)}$; a change in either sector's productivity $a$ enters the same way, and the scenarios hold both fixed.
