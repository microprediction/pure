...
The reinventions are one object under two names. A second failure says more about abstraction itself. A theorem about $A$ is known. An instance $A'$ of $A$ turns up in another field, and the corresponding $B'$ follows at no cost, but nobody recognizes $A'$ as an instance of $A$, so $B'$ is proved again by hand or left open. Figure~\ref{fig:descent} draws five cases. The minimum distance of a linear code, which is the girth of a vector matroid, was proved NP-hard in 1997; spark is the same invariant over the reals, and compressed sensing proved it NP-hard separately in 2014. The collapsed-strata variance estimator and its bias theorem date from 1953; a 2025 paper on finely stratified experiments derived both again. Additive segment-cost objectives have had an exact quadratic-time solution since 1989; a 2024 paper solved one by mixed-integer programming and called exact optimality its contribution. The relative-entropy variational identity is 1970s large deviations; a 2026 control paper proved its control-affine instance as a new equivalence. Against these, Grossmann recognized Einstein's problem as an instance of Riemannian geometry in 1912, and the whole theory transferred at once. Recognizing an instance is the act of abstraction, and the drop in the figure is free for anyone who performs it.
...
In every case the result had been published, indexed, and searchable for years. The missing piece was the recognition that two objects described in different vocabularies were one object, and until recently only a person could supply it. That recognition is abstraction, not retrieval, and it is now available on demand: a model that has read four literatures can be asked whether spark and girth are the same thing and answer with the identity. Every earlier information technology, from the journal to the search engine, organized text and left the abstraction to the reader. This is the part of the change most likely to be undercounted, because it is the part no earlier technology touched.

Whether a model can supply that recognition is testable. For each of the nine cases, a fresh instance of a current model was asked the question a practitioner would ask from inside one field, using only that field's vocabulary: is this quantity, or this procedure, known under another name elsewhere, and what is the exact relationship? The other literature was not mentioned, web search was switched off, and the answer was limited to 150 words. All nine answers named the other literatures and stated the relationship, usually with the identity and the dates. Asked about spark, the model gave girth and Kruskal rank and the equation connecting them; asked about raking, it gave iterative proportional fitting, RAS, Sinkhorn scaling, and the information-projection reading; asked about kriging, it gave the Gaussian-process posterior mean and the Wiener-Kolmogorov filter. The prompts and answers are recorded with the paper. One limit should be stated: all nine identifications exist in literature the model has read, so the test shows a model carrying a known identification across a vocabulary boundary on request, which was the bilingual person's job, and not making a new one.

\section{What is known about the return}

\citet{jones2020} compute the average economy-wide social return to innovation investment and find a benefit-cost ratio above ten under central parameters, at least four under conservative ones, and above twenty once international spillovers and health benefits are counted. The estimate is for research and development as a whole, not mathematics, but mathematics sits at the upstream end of the chain, where the spillovers are largest. \citet{deloitte2013} attribute 2.8 million UK jobs and \pounds 208 billion of gross value added, about sixteen per cent of the total, to occupations that use tools derived from mathematical-science research. The Academy for the Mathematical Sciences updated the figure to \pounds 495 billion for 2023 \citep{acadmathsci2024}. These are gross measures with an occupational attribution rule, not marginal returns to another pound of research, and they bound the total from above rather than estimate the return to one more pound.

\citet{nelson1959} and \citet{arrow1962} argued that private markets underinvest in basic research because the returns cannot be appropriated by the investor. \citet{romer1990} made the mechanism explicit: ideas are non-rival, so their social value scales with the number of users while the private value does not. The argument is older than large language models and does not depend on them. It implies that spending set by private returns, absent a policy correction, falls below the social optimum. \citet{akcigit2021} estimate a growth model in which basic and applied research generate different spillovers and find that a uniform research subsidy oversubsidizes applied research, while support for public basic research is welfare-improving. \citet{mansfield1991} surveyed firms and found that about a tenth of their new products and processes could not have been developed without recent academic research, and put the social rate of return to that research near thirty per cent. \citet{salter2001} review the wider evidence and reach the same conclusion.
...
Written down, the value of a result is the number of uses it reaches, times the net value of a use, times a discount for the wait, and the three channels multiply within a segment of uses and average across segments. Appendix A writes it down as a minimal talking point rather than a model: a bookkeeping identity and a power-law planner, chosen to be transparent rather than realistic. Appendix C sets the channels at low, central, and high values, and the value multiplier that comes out is 8, 190, or 36{,}000. The direction is the same in every column because every channel moves the same way, given cheap screening and the marginal transport assumption; the size is illustrated, not measured.

\section{Optimal investment}

Let a planner spend $S$ dollars on unmotivated mathematics and obtain $I(S) = a S^{\gamma}$ results, $0 < \gamma < 1$, each of expected value $V$. Here $V$ is the marginal value of a newly produced result, conditional on the stock already reachable through the model. The model also unlocks that stock, which raises the level of realized value and may substitute for some new production. The multiplier carries over to the margin by assumption.
...
\section{The declaration, answered}
...
This too can be tested. Two recent proofs, each over a hundred pages and by reputation hard going, Wang and Zahl's proof of the Kakeya conjecture in three dimensions \citep{wangzahl2025} and Baek's proof that Gerver's sofa is optimal \citep{baek2024}, were given to a model with the instruction to extract one idea that a strong non-specialist could follow, that was new in the paper, with a toy case and a candidate use elsewhere. From the first it returned the factoring proposition, the idea the paper itself names as new, with a toy version in the plane and a reading of it as a regularity lemma for point sets with a greedy algorithm attached. From the second it returned the injectivity condition, with the eleven-step iteration that proves it presented as an undergraduate exercise, and the observation that a piano-mover's obstacle corner traces the same curve. In both it separated what was new from what was inherited and said which sections it had read. That is the textbook end of the journey, done in an hour, on two proofs whose textbook chapters do not yet exist.
